...
int score_letter(char letter, char **vocabulary, size_t num_words) {
  ...
  for (size_t x = 0; x < num_words; x++) {
    ...
    char *temp = vocabulary[x];
    if(letter == temp[0] || letter == temp[1] || letter == temp[2] || letter == temp[3] || letter == temp[4])
      words++;;
  }
//  printf("%zu\n", words);
...
// score once.
  int score_word(char *word, int *letter_scores) {
    int sum = letter_scores[word[0] - 'a'];
    int x ;
